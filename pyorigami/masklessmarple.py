...
class Folder(Talker):
    ...
    def gridify(self):
        ...
        self.grid['depths'] = np.zeros((self.ndurations, self.ngrid))
        self.grid['depths'][:, self.indices] = self.depths

        self.grid['uncertainties'] = np.zeros((self.ndurations, self.ngrid))
        ...
        self.grid['inversevariances'][self.grid['uncertainties'] == 0] = 0.0
